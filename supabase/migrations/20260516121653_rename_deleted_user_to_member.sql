-- Brand consistency: anonymized accounts read as "Deleted member" everywhere
-- to match the in-app "member" vocabulary. Previously the RPC wrote
-- 'Deleted user' which clashed with the info-card copy that promised
-- "shown as 'Deleted member'".
...
-- One-off backfill (idempotent): rename anyone already anonymized under the
-- previous copy so they're consistent.
UPDATE public.users
