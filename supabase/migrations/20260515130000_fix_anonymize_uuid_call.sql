-- Mirrors the runtime fix applied to prod after the original
-- account_deletion_anonymization migration shipped: anonymize_user_account
-- called uuid_generate_v4(), which lives in the `extensions` schema and is
-- not reachable from a SECURITY DEFINER function with SET search_path = public.
-- Switched to gen_random_uuid() (built-in to Postgres core).
--
-- This file is redundant against the (now-corrected) source migration but is
-- kept so the local migrations folder matches the migration history in the
-- database, where this fix was applied as a separate entry.
